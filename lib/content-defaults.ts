...
export const DEFAULT_VALUES = {
  ...
  cards: [
    ...
    { title: t("Unser Stil", "Our style"), description: t("Ein charakteristisches Merkmal unserer Häuser ist das Fassadenmauerwerk im **Bayerischen Stil***. Die unterschiedlichen Farbtöne der Ziegel verleihen jedem Gebäude eine individuelle Ausstrahlung und schaffen ein harmonisches Gesamtbild, das auch über viele Jahre hinweg seinen zeitlosen Charakter bewahrt.", "A characteristic feature of our houses is the **Bavarian Masonry*** facade brickwork. The varying shades of the bricks give each building an individual character and create a harmonious overall look that retains its timeless character over many years.") },
  ],
  // Footnote shown directly below the four value cards. The leading "*" is the
  // reference marker for "Bayerischen Stil*" in the "Unser Stil" card; only the
  // term itself is bold (**…** markup, rendered by components/about.tsx).
  footnote: t(
    "* **Bayerischer Stil** ist eine künstlerische Fassadenverkleidung, die auf einer unregelmäßigen Kombination von Klinkersteinen in 4–8 Farbtönen basiert — gewöhnlich von Rot bis Dunkelbraun — und dadurch ein farbenreiches, ästhetisches Muster erzeugt. Diese Technik entstand in Deutschland, insbesondere in Bayern, im 17. und 18. Jahrhundert. Aufgrund der damaligen Unvollkommenheit der Brennöfen war es nicht möglich, eine gesamte Ziegelcharge in identischer Farbe zu brennen. Daher mussten die Bauleute Ziegel aus verschiedenen Chargen miteinander mischen, was sich im Laufe der Zeit zu einem wiedererkennbaren Baustil entwickelte.",
    "* **Bavarian Masonry** is an artistic façade cladding based on an irregular combination of clinker bricks in 4–8 shades — usually ranging from red to dark brown — creating a colorful and aesthetically distinctive pattern. This technique originated in Germany, especially in Bavaria, during the 17th and 18th centuries. At that time, because of the limitations of the kilns, it was not possible to fire an entire batch of bricks in an identical color. Builders therefore had to mix bricks from different batches, which over time developed into a recognizable architectural style.",
  ),
}
